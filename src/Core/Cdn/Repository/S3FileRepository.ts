...
export default class S3FileRepository {
  constructor(
    private readonly uniqidGenerator: UniqidGenerator,
  ...
  private async createUpload(dto: UploadFileDto) {
    return {
      Bucket: dto.path,
      Key: `${this.uniqidGenerator.generate()}.${this.extractExtensionFromFilename(
        dto.file,
      )}`,
  ...
  private makePublicUrl(upload: any) {
    return `${this.configService.get("CDN_PUBLIC_URL")}/${upload.Bucket}/${
      upload.Key
    }`;
  }
}
