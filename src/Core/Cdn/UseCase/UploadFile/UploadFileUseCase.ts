...
export default class UploadFileUseCase
  implements UseCase<Promise<FileEntity>, [dto: UploadFileDto]>
{
  constructor(
  ...
  async handle(context: ContextualGraphqlRequest, dto: UploadFileDto) {
    try {
      const file = await this.prismaFileRepository.create(
        await this.s3FileRepository.create(dto),
      );
      ...
      this.eventEmitter.emit("upload_file_failed", {
        context,
        dto,
        error: error.message,
      });
