...
@Injectable()
export default class RemoveTaskUseCase implements UseCase<Promise<Task>, [taskId: number]> {
    ...
    handle(context: ContextualGraphqlRequest, taskId: number) {
        try {
            ...
            throw new BadRequestException("no task has been created please create a new one ",error.message);
        }
    }
}