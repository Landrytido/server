...
@Injectable()
export default class UpdateLinkUseCase implements UseCase<Promise<Link>, [id: number, dto: UpdateLinkDto]> {
    ...
    async handle(context:ContextualGraphqlRequest, id: number, dto: UpdateLinkDto): Promise<Link> {
        try {
            ...
            if(link.userId !== context.userId)
                throw new InsufficientPermissionException("You do not have permission to access or modify this link.");
