...
import {CalendarEvent, CalendarEventType, NotificationPreference, NotificationType, TimeUnit} from "@prisma/client";
...
export class SendCalendarEmailNotificationJob {
    ...
    async handle() {
	  this.eventNotificationSent = 0;
	  this.taskNotificationSent = 0;
	  const now = new Date();
    ...
    private async processNotifications(items: CalendarEventWithRelations[], now: Date) {
	  for (const item of items) {
		...
		// @ts-ignore
		const hasPush = (item.notificationPreference)?.types?.some(
			(t) => t.type === NotificationType.EMAIL
		);
		if (!hasPush) {
		...
		if (notificationPreference) {
		    ...
		    if (now >= reminderTime && item.emailNotificationSent === false) {
			  ...
			  // this.logger.log(
				//   `Notification envoyée pour ${item.eventType} (id:${item.id}) ${item.eventType} : ${item.title}`,
			  // );
